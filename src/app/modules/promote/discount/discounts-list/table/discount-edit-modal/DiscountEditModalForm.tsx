...
const editDiscountSchema = Yup.object().shape({
  ...
  value: Yup.string()
    ...
    .test('value-type', '100%-с илүү дүн оруулж болохгүй.', function (value, ctx) {
      let res = true
      if (ctx.parent.type === 'percent') res = value && parseInt(value) > 100 ? false : true
      return res
    })
...
const DiscountEditModalForm: FC<Props> = ({discount, isLoading}) => {
  ...
  const [typeClassName] = useState(
    'btn btn-outline-secondary text-muted text-hover-white text-active-primary btn-outline btn-active-success'
  )
  const [typeClassNameActive] = useState(
    'btn btn-outline-secondary text-muted text-hover-white text-active-primary btn-outline btn-active-success active'
  )
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, {setSubmitting}) => {
      ...
      try {
        var response: any = {}
        if (isNotEmpty(values.id)) response = await updateDiscount(values)
        else response = await createDiscount(values)
    ...
    <>
        ...
        >
          ...
          <div className='row mb-6'>
            ...
            <div className='fv-row col-4 col-lg-3'>
              ...
              >
                <label
                  className={type === 'percent' ? typeClassNameActive : typeClassName}
                  data-kt-button='true'
                >
                  <input
                    className='btn-check text-active-primary'
                    type='radio'
                ...
                <label
                  className={type === 'cash' ? typeClassNameActive : typeClassName}
                  data-kt-button='true'
                >
                  <input
                    className='btn-check'
            ...
            <div className='fv-row col-8 col-lg-9'>
              {/* <label className='d-block fw-bold fs-6 mb-2' style={{color: 'white'}}>
                {' '}
                .
              </label> */}
              ...
              {((formik.touched.type && formik.errors.type) ||
                ...
                <div className='fv-plugins-message-container'>
                  <div className='fv-help-block'>
                    {formik.errors.value ? formik.errors.value : formik.errors.type}
                  </div>
                </div>
              )}
            </div>
          ...
          <div className='row mb-6'>
            <div className='fv-row col-6'>
              ...
              <Datetime
                ...
                onChange={(val) => {
                  handleOnChangeDate('start_date', val)
                }}
              />
            ...
            <div className='fv-row col-6'>
              ...
              <Datetime
                ...
                onChange={(val) => {
                  handleOnChangeDate('end_date', val)
                }}
              />
          ...
          <div className='fv-row mb-6'>
            ...
            <div className='input-group'>
              ...
              />
              <span
                className='input-group-text text-hover-primary'
                id='basic-addon2'
      ...
      {openServiceModal && (
        <ServicesModal
          ...
          selectedServices={formik.values.selected_services}
