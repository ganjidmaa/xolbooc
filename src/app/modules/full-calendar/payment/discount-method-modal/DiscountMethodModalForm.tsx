...
const discountMethodSchema = Yup.object().shape({
  amount: Yup.string()
    ...
    .test('amount-type', '100%-с илүү дүн оруулж болохгүй.', function (amount, ctx) {
      let res = true
      if (ctx.parent.type === 'percent') res = amount && parseInt(amount) > 100 ? false : true
      return res
    })
...
const DiscountMethodModalForm: FC<Props> = ({
  toggleDiscountModal,
  ...
  const [discountInitialValue] = useState<Discount>({
    amount:
      (discountList[0]?.type === 'percent' ? discountList[0]?.percent : discountList[0]?.amount) ||
      '',
              ...
              {((formik.touched.type && formik.errors.type) ||
                ...
                <div className='fv-plugins-message-container'>
                  <div className='fv-help-block'>
                    {formik.errors.amount ? formik.errors.amount : formik.errors.type}
                  </div>
                </div>
              )}
            </div>
