...
const ItemDetail: FC<Props> = ({item, index, viewType}) => {
  ...
  const removeItem = () => {
    setItemDatas((prevData) => {
      return prevData.filter((item, seq) => seq !== index)
    })
  }

  return (
    <div className='d-flex align-items-center mb-4'>
      <div className='text-gray-800 fs-6' style={labelStyle.style}>
        {item.start_time}
      </div>
      {/* <span className='bullet bullet-vertical h-45px bg-primary me-5'></span> */}
      <span className='bullet bullet-vertical h-45px bg-info me-5'></span>

      <div className='flex-grow-1'>
        <div className='text-gray-800 fw-bolder fs-6'>
          {/* {data.service_obj?.label} */}
          {data.service_name}
        </div>
        <span className='text-muted fw-bold d-block'>
          ...
          <br />
          {/* {settings?.has_service_type && data.service_type_name} */}
        </span>
