...
export const EventPage = () => {
  ...
  const {refetch, data: eventDatas} = useQuery(
    `${QUERIES.EVENT_DETAIL}-${eventIdForUpdate}`,
    () => {
  ...
  useEffect(() => {
    if (eventDatas) {
      ...
      if (selectedCustomer) {
        setEventCustomer(selectedCustomer)
      }
      ...
      let type =
        status === 'completed' || status === 'part_paid' || status === 'unpaid' ? 'view' : 'edit'
  ...
  return (
    <div className='form d-flex flex-column flex-lg-row'>
      {asideType === 'edit' && event && (
        <EventEditDetail event={event} viewType={asideType} changeAsideType={changeAsideType} />
      )}

      {asideType !== 'edit' && (
        <>
          <div className='flex-column flex-lg-row-auto gap-7 gap-lg-10 w-lg-450px w-xl-500px mb-7 me-lg-5'>
            <div className='card card-flush pb-4 pt-2 h-lg-600px'>
